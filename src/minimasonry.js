...
MiniMasonry.prototype.getCount = function () {
    if (this.conf.surroundingGutter) {
        return Math.floor((
                              this._width - this.conf.gutterX
                          ) / (
                              this.conf.baseWidth + this.conf.gutterX
                          ));
    }
...
MiniMasonry.prototype.layout = function () {
    ...
    if (this._count > this._sizes.length && this.conf.center) {
        //If more columns than children and center is true
        var occupiedSpace = (
                                this._sizes.length * (
                                width + this.conf.gutterX
                                                   )
                            ) - this.conf.gutterX;
        initialLeft = (
            (
                this._width - occupiedSpace
            ) / 2
        );
    }
    ...
    for (var index = 0; index < children.length; index++) {
        ...
        this._columns[nextColumn] += this._sizes[index] + (
            this._count > 1 ? this.conf.gutterY : this.conf.ultimateGutter
        );//margin-bottom
    }

    this._container.style.height = (
                                       this._columns[this.getLongest()] - this.conf.gutterY
                                   ) + 'px';
};
